...
# Declares its two events by the two available routes — a plain overload and one
# generated by the apply macro — so `handles?` is exercised against both. The
# macro-generated def only reaches `handles?` if `@type.methods` sees it.
class TestHandlesDeclarationProjection < ES::Projection
  include ES::ProjectionDSL
...
describe "ES::Projection.handles?" do
  it "reports true for every declared event, however it was declared" do
    TestHandlesDeclarationProjection.handles?(DummyEvent).should be_true
    TestHandlesDeclarationProjection.handles?(OtherDummyEvent).should be_true
  end

  it "reports false for an undeclared event" do
    TestApplyDSLProjection.handles?(DummyEvent).should be_true
    TestApplyDSLProjection.handles?(OtherDummyEvent).should be_false
  end

  it "reports true for any event when the catch-all is overridden" do
